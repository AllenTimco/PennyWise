...
// smartspectra_config_t — struct_size gates appended optional fields.
koffi.struct('smartspectra_config_t', {
    api_key:                   'const char *',
    requested_metrics:         'const int32_t *',
    requested_metrics_len:     'int',
    enable_accumulated_output: 'int',
    log_level:                 'int',       // a SMARTSPECTRA_LOG_LEVEL_* constant (0 = SDK default)
    struct_size:               'uint32_t',  // sizeof(smartspectra_config_t); gates appended fields
    disable_telemetry:         'int',       // 0 = telemetry on (default), non-zero = opt out
    reserved0:                 'int',
});
...
class Session {
     ...
     * @param {number} [cfg.logLevel] SmartSpectraLogLevel value; omit for the SDK default (warning)
     * @param {boolean} [cfg.enableTelemetry=true] Opt out of aggregate SDK telemetry by passing false.
    ...
    constructor(cfg, callbacks) {
        ...
        // Held from a successful start*() until native teardown completes
        // (_teardownNative(), the destroy() path) — NOT released by
        // stop()/stopAsync()/reset(). Those calls can return while a worker
        // thread is still mid-relay of a trailing callback (e.g. the idle
        // status transition after stop()); only session_destroy's joins
        // guarantee no SDK thread can still call our trampolines. koffi's
        // registered thunks do NOT ref the libuv loop, so without this a host
        // whose only pending work is awaiting an SDK-callback-resolved
        // promise drains its loop, Node disposes the isolate, and the next
        // relayed callback segfaults the process (exit 139).
        ...
        const cConfig = {
            ...
            // C ABI wire encoding (SMARTSPECTRA_LOG_LEVEL_* in
            // smartspectra_capi.h): 0 keeps the SDK default; explicit levels
            // are SmartSpectraLogLevel + 1 so a zeroed struct never means
            // "most verbose".
            log_level:                 Number.isInteger(cfg.logLevel) ? cfg.logLevel + 1 : 0,
            struct_size:               koffi.sizeof('smartspectra_config_t'),
            disable_telemetry:         cfg.enableTelemetry === false ? 1 : 0,
            reserved0:                 0,
        };
    ...
    destroy() {
        ...
        if (this._stopAsyncPromise) {
            ...
            this._teardownPromise = this._teardownNative();
        }
        return this._teardownPromise;
    }

    // Returns a promise that resolves once native teardown (including any
    // worker-thread joins inside session_destroy) has completed.
    _teardownNative() {
        if (this._sess === null) {
            this._releaseKeepAlive();
            return Promise.resolve();
        }
        // session_destroy performs the required best-effort stop and worker
        // joins. Run that entire drain off the JS thread: an SDK worker may be
        // blocked relaying a callback onto this event loop, so synchronously
        // stopping or waiting here would deadlock both threads. Unregister the
        // thunks only after destroy has joined every possible caller.
        const sess = this._sess;
        this._sess = null;
        return new Promise((resolve) => {
            sessionDestroy.async(sess, () => {
                this._releaseThunks();
                this._releaseKeepAlive();
                resolve();
            });
    ...
    _takeKeepAlive() {
        ...
        // Max-int32 ms (~24.8 days): the timer exists only to hold a loop
        // reference; it never meaningfully fires.
...
// Internal one-time SDK initialization. Called directly by the Electron
// main-process glue in `@smartspectra/node-sdk/main` (which namespaces its
// own cache directory under app.getPath('cache')). Not part of the package's
// public exports map — host apps don't reach for it; use ensurePreconfigured()
// below for the plain-Node default instead.
...
// Per-OS writable cache directory, used as the default when nothing else
// (e.g. Electron's app.getPath('cache')) has already resolved one.
function defaultSdkDirectory() {
    ...
    return path.join(base, 'SmartSpectraSDK');
}

let autoPreconfigureAttempted = false;

// Idempotent, best-effort preconfigure for hosts that never call preconfigure()
// themselves. ARTERIAL_PRESSURE_TRACE, EDA_TRACE, and micromotion metrics need
// the device-key cache directory Preconfigure() sets up (smartspectra.h); without
// it their encrypted-model load fails downstream and Session.start*() throws —
// only Electron's main-process glue (js/main/index.js) called this previously,
// so a plain Node host requesting one of those metrics had no way to succeed.
// Runs once per process; failures are logged, not thrown, so metrics that don't
// need EMD still work. Called from SmartSpectraSDK._ensureSession() so every
// entry point — plain Node or Electron — gets it before a session can start;
// Preconfigure() itself only resolves the directory/device-identity once, so a
// prior, more specific call (e.g. Electron's app-namespaced one) always wins.
function ensurePreconfigured() {
    if (autoPreconfigureAttempted) return;
    autoPreconfigureAttempted = true;
    const sdkDirectory = defaultSdkDirectory();
    try {
        // nosemgrep: detect-non-literal-fs-filename - safe: derived from OS-standard cache path env vars / os.homedir(), not untrusted input.
        fs.mkdirSync(sdkDirectory, { recursive: true, mode: 0o700 });
        preconfigure(sdkDirectory);
    } catch (err) {
        console.error(
            `SmartSpectra: automatic preconfigure failed (${err.message || err}). ` +
            'Arterial pressure, EDA, and micromotion metrics will not be available ' +
            `until preconfigure() succeeds for ${sdkDirectory}; other metrics still work.`);
    }
}
