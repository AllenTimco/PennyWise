...
const { Session, ensurePreconfigured } = require('./ffi');
const {
    ProcessingStatus,
...
class SmartSpectraSDK {
     ...
     * @param {number} [options.logLevel] SmartSpectraLogLevel value applied when the
     *   session initializes; omit for the SDK default (SmartSpectraLogLevel.kWarning)
    ...
    _ensureSession() {
        if (this._session) return;
        // ARTERIAL_PRESSURE_TRACE, EDA_TRACE, and micromotion metrics need the
        // EMD cache directory Preconfigure() sets up before the graph starts;
        // best-effort and idempotent, so plain metrics work even if it fails.
        ensurePreconfigured();
