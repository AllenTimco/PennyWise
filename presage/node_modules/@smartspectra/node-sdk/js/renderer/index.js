...
const { FrameTimestampAnchor } = require('./timestamp');
...
class SmartSpectraSDK {
     ...
     * @param {boolean} [options.enableTelemetry=true] Aggregate SDK telemetry; set false to opt out.
    ...
    constructor(options = {}) {
        ...
        // Epoch-anchor + monotonic-ratchet for VideoFrame.timestamp. Lives on
        // the instance (not the frame loop) so it persists across
        // stop()/start() cycles: the main process reuses one native session
        // whose per-session monotonic guard is NOT reset by stop/start, so a
        // fresh camera clock starting lower than the previous session's last
        // timestamp would otherwise trip kNonMonotonicTimestamp. See
        // ./timestamp.js for why the epoch anchor is needed at all.
        this._frameTimestamps  = new FrameTimestampAnchor();
        ...
        try {
            while (this._frameLoopActive && !this._destroyed) {
                ...
                try {
                    ...
                    // VideoFrame.timestamp is already in microseconds, but in
                    // the frame's own media-clock domain — anchor it to the
                    // Unix epoch and ratchet it monotonic (see
                    // ./timestamp.js).
                    const ts = this._frameTimestamps.next(vf.timestamp);
