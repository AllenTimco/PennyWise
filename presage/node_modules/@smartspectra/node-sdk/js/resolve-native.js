...
// Per-platform payloads are named `<main package>-<plat>-<arch>`. The prefix
// is read from this package's own manifest (not hardcoded) so a publish
// variant that rewrites package.json#name resolves its own payload packages.
const MAIN_PKG_NAME = require('../package.json').name;

function platformPackageName() {
    return `${MAIN_PKG_NAME}-${platformArch()}`;
}
...
function resolveNativeLibrary() {
    ...
    try {
        ...
        throw new Error(
            ...
            `It is a regular dependency of ${MAIN_PKG_NAME}, so this usually means your ` +
